...
class TiffFilter:
    ...
    @staticmethod
    def filter_tiff(tiff_array: np.array, init_threshold: float, null_value=0):
        return np.where(tiff_array <= init_threshold, tiff_array, null_value)
    ...
    def multiprocessing_function(self, file, init_threshold=0.8, footprint=(50, 50), area_threshold=5000):
        img = rasterio.open(file)
        arr = img.read()[0]
        arr = self.filter_tiff(arr, init_threshold*np.max(arr), 0)
        arr[arr > 0] = 1
    ...
    def run(self, n_cpus=4, init_threshold=0.8, footprint=(50, 50), area_threshold=5000):
        tqdm_pathos.map(
    ...
    def split_files(self, files: list, filtered=True) -> dict:
        """
